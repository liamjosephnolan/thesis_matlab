...
% Time trimming parameter (in seconds)
time_trim = 30; % Set this to your desired trim time

%% Define roll and pitch grid values (manually ordered)
roll_vals = [-20, 0, 20];
pitch_vals = [-15, 0, 15];  % Note: exclude -3 since you want a clean 3x3
...
gridPoints = [
    struct('file', 'prbs_roll_m20_pitch_m15.csv', 'roll', -20, 'pitch', -15, ...
           'row_idx', 1, 'col_idx', 1, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1);
    struct('file', 'prbs_roll_0_pitch_m15.csv', 'roll', 0, 'pitch', -15, ...
           'row_idx', 1, 'col_idx', 2, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1);
    struct('file', 'prbs_roll_20_pitch_m15.csv', 'roll', 20, 'pitch', -15, ...
           'row_idx', 1, 'col_idx', 3, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.01);
    struct('file', 'prbs_roll_m20_pitch_0.csv', 'roll', -20, 'pitch', 0, ...
           'row_idx', 2, 'col_idx', 1, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1)
    struct('file', 'prbs_roll_0_pitch_0.csv', 'roll', 0, 'pitch', 0, ...
           'row_idx', 2, 'col_idx', 2, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1);
    struct('file', 'prbs_roll_20_pitch_0.csv', 'roll', 20, 'pitch', 0, ...
           'row_idx', 2, 'col_idx', 3, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1);
    struct('file', 'prbs_roll_m20_pitch_15.csv', 'roll', -20, 'pitch', 15, ...
           'row_idx', 3, 'col_idx', 1, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1);
    struct('file', 'prbs_roll_0_pitch_15.csv', 'roll', 0, 'pitch', 15, ...
           'row_idx', 3, 'col_idx', 2, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1);
    struct('file', 'prbs_roll_20_pitch_15.csv', 'roll', 20, 'pitch', 15, ...
           'row_idx', 3, 'col_idx', 3, 'Qpos', 600, 'Qvel', 0.25, 'Qi', 0.01, 'R', 0.1);
];
...
if runMode == 0
    ...
    for i = 1:numel(gridPoints)
        ...
        % Trim data to specified time
        trim_idx = t <= time_trim;
        t = t(trim_idx);
        pos = dataTbl{trim_idx,7};
        u = dataTbl{trim_idx,8};
        Ts = mean(diff(t));
...
if runMode == 0 || runMode == 1
    ...
    fprintf('Data trimmed to %.1f seconds\n', time_trim);
    for i = 1:numel(results)
        fprintf('\nOperating Point %d:\n', i);
